...
public class Path
{
    ...
    public Path(Vector2 firstAnchor, Vector2 secondAnchor)
    {
        points = new List<Vector2>
        {
           firstAnchor,
           firstAnchor + (Vector2.right + Vector2.up)*.5f,
           secondAnchor + (Vector2.left + Vector2.down)*.5f,
           secondAnchor
        };
    }
    ...
    public int NumSegments
    {
        get
        {
            return points.Count/ 3; //Should have just done (Count-1)/3
        }
    }
    ...
    public void UpdatePoints(Vector2 oldPos, Vector2 newPos)
    {
        Vector2 decalage = newPos - oldPos;
    ...
    public void RelocatePath(Vector2 displacement)
    {
        for (int i = 0; i< points.Count; i++)
        {
