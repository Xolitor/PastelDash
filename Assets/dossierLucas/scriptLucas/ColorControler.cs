...
{   
    ...
    public float smoothnessBoss = 0.51f;
    ...
    public IEnumerator SwitchRandomColor() // Méthode de changement progressif de la couleur du terrain 
    {
        ...
        while (!rend.material.color.Equals(nextColor)) // tant que la couleur suivante n'est pas atteinte
        {
            progress += increment;
            rend.material.color = Color.Lerp(prev, nextColor, progress); // Changement progressif de la couleur vers la couleurs objectif
            yield return null;
        }
        yield return null;
    }
